...
exports.extractDataFromMessage = (webMessage) => {
  ...
  console.log("[LYNX GAMING | DEBUG] Mensaje completo:", fullMessage); // Añadir log
  ...
  console.log("[LYNX GAMING | DEBUG] Datos extraídos:", {
    args,
...
exports.findCommandImport = (commandName) => {
  ...
  console.log(`[LYNX GAMING | DEBUG] Comando encontrado: ${commandName}`); // Verifica que el comando esté siendo encontrado
...
exports.readCommandImports = () => {
  const subdirectories = fs
    ...
    .filter((directory) => directory.isDirectory()) // Filtramos solo las subcarpetas
    .map((directory) => directory.name);

  const commandImports = {};

  // Recorremos cada subcarpeta (owner, admin, member)
  for (const subdir of subdirectories) {
    ...
    const files = fs
      ...
      .filter(
        (file) =>
          !file.startsWith("_") && // Filtramos los archivos que no empiezan con "_"
          (file.endsWith(".js") || file.endsWith(".ts")) // Solo archivos .js o .ts
      )
      .map((file) => require(path.join(subdirectoryPath, file))); // Cargamos los archivos

    commandImports[subdir] = files; // Guardamos los comandos por subcarpeta
  }
